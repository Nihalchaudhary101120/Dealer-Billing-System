...
// import tvsLogoPath from "../../assets/tvs.svg"
...
.hdr-mid   { font-size:11px; font-weight:bold;  text-align:center; }
.hdr-right { font-size:11px; text-align:right; font-weight:bold; }
...
.logo-col { min-width:100px; text-align:right; }
...
<div class="dealer-addr-row">
  <div class="dealer-blk">
    <div class="dealer-name">${dealer.branchName || ""}</div>
    ${dealer.address ? dealer.address.split(",").map(a => `<div>${a.trim()}</div>`).join("") : ""}
    ${dealer.state ? `<div>${dealer.state}</div>`: "<div>state:</div>"}
    ${dealer.phone ? `<div>Ph: ${dealer.phone}</div>` : "<div>Ph:</div>"}
    ${dealer.gstNumber ? `<div>GST IN No.:${dealer.gstNumber}</div>` : ""}
  </div>
          ...
          <div style={pageStyle}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginTop: "28px", fontSize: "10px" }}>
              ...
              <div style={{ textAlign: "right", lineHeight: "1.8" }}>
                <div>For {dealer?.branchName || ""}</div>
                <div style={{ marginTop: "15px" }}>Authorised Signatory</div>
              </div>
            </div>
